#/usr/bin/R
# to run from shell:
# R CMD BATCH plot2.R ; cat plot2.Rout


data  <- read.csv("household_power_consumption_short.txt",sep=";")
png(filename="plot4.png", width = 480, height = 480)

par(mfrow = c(2, 2))

#1
plot(data$Global_active_power,type='l',xaxt="n",ylab='Global Active Power',xlab="")
axis(1,at=c(1,1441,2880),labels=c("Thu","Fri","Sat"))


#2
plot(data$Voltage,type='l',xaxt="n",ylab='Voltage',xlab="datetime",col="black")
axis(1,at=c(1,1441,2880),labels=c("Thu","Fri","Sat"))

#3
plot(data$Sub_metering_1,type='l',xaxt="n",ylab='Energy sub metering',xlab="",col="black")
lines(data$Sub_metering_2, col="red")
lines(data$Sub_metering_3,col="blue")
axis(1,at=c(1,1441,2880),labels=c("Thu","Fri","Sat"))
# legend("topright",  col = c("black", "red", "blue"), legend = c("sub_metering_1", "sub_metering_2", "sub_metering_3"), pch=2)
legend("topright",  col = c("black", "red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=c(1,1),bty = "n")

#4
plot(data$Global_reactive_power,type='l',xaxt="n", ylab='Global_reactive_power',xlab="datetime",col="black")
axis(1,at=c(1,1441,2880),labels=c("Thu","Fri","Sat"))



dev.off()

